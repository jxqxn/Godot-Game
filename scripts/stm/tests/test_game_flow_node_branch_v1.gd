...
func test_game_flow_fourth_floor_rest_unlocks_combat_and_event_fifth_floor_nodes() -> void:
	...
	# Then：第 4 层后是两个第 5 层节点，分别为 combat / event。
...
func test_game_flow_event_branch_reaches_fifth_floor_event_then_merges_to_sixth_floor() -> void:
	# Given：第 4 层休息完成后。
	var flow = _flow_at_completed_fourth_floor_rest()
	# When：选择第 5 层 node 1 事件分支。
	assert_true(flow.advance_to_next_node(4, 1))
	assert_eq(flow.get_current_floor_index(), 4)
	assert_eq(flow.get_current_node_index(), 1)
	assert_true(flow.enter_current_room())
	# Then：进入的是第 5 层 event，完成前不能前往第 6 层。
	...
	assert_false(flow.advance_to_next_node(5, 0))
	# When：通过 GameState.submit_choice() 完成 event_choice。
	assert_true(_leave_pending_event_choice(flow))
	var next_nodes: Array = flow.get_available_next_nodes()
...
func test_game_flow_node_path_can_reach_boss_after_event_branch_and_sixth_floor() -> void:
	# Given：选择第 5 层 event 分支并汇合到第 6 层。
	var flow = _flow_at_completed_fourth_floor_rest()
	assert_true(flow.advance_to_next_node(4, 1))
	assert_true(flow.enter_current_room())
	assert_true(_leave_pending_event_choice(flow))
