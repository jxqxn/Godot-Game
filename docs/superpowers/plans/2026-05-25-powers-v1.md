...
- 本阶段不把 `Bash`、`Inflame`、`Shrug It Off` 加入固定调试战斗牌组；策划调试按钮仍只覆盖 `Strike`、`Defend`、结束回合、重开战斗。
...
- 默认执行方式：Subagent-Driven。
- 每个 Task 使用 fresh 实现代理，实施模型使用 `gpt-5.3-codex`。
- 每个 Task 实现完成后进行双重审核，审核模型使用 `gpt-5.5`。
- 每个 Task 都按 RED-GREEN 顺序执行：先写中文 BDD 测试，再运行失败测试，再写实现，再运行通过测试。
- 本计划中的 `all current tests` 明确指“该 Task 完成后 `.gutconfig.json` 中列出的全部测试路径”。
- 本计划中的 `minimal implementation` 明确指“以该步骤代码块为准，只补足当前文件所需的上下文缩进和相邻签名，不扩展未列出的功能”。
- RED 阶段出现预期中的缺失脚本、缺失类或缺失构造参数错误时，只记录失败证据，不提前跳过测试步骤。
...
Expected: PASS for `core_skeleton_test.gd`, `test_battle_debug_scene.gd`, `test_fixed_battle_fixture.gd`, and these three `test_powers_v1.gd` tests: `test_apply_power_stacks_intensity_or_duration`、`test_strength_increases_attack_damage`、`test_vulnerable_increases_damage_taken`。
...
Expected: PASS for every test path listed in `.gutconfig.json` after Task 2 edits.
...
Expected: PASS for every test path listed in `.gutconfig.json` after Task 3 edits.
...
Expected: PASS for every test path listed in `.gutconfig.json` after Task 4 edits.
...
Expected: PASS for every test path listed in `.gutconfig.json` after Task 5 edits.
...
- [ ] **Step 4: Commit final verification edits only when files changed**

Trigger condition: Step 3 produced code、test、config 或 plan edits and `git status --short` shows modified files. When Step 3 is inspection-only and the worktree is clean, record “无需额外提交” in the final handoff instead of running this commit step.
