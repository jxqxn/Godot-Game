...
- 允许先补齐项目测试入口所需的 GUT 插件，使 `addons/gut/gut_cmdln.gd` 存在。除 GUT 外，不要联网下载其他依赖，不要引入其他外部 Godot 插件。
...
- GUT 是本阶段允许补齐的测试依赖，必须先确保 `addons/gut/gut_cmdln.gd` 存在。除此之外，不新增第三方 Godot 插件，也不临时改用另一套测试入口。
...
执行实现前必须先补齐 `addons/gut/gut_cmdln.gd`。补齐后所有测试仍统一使用上述命令。
