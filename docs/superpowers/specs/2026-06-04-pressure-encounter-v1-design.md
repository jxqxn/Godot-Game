...
```text
玩家在一个压力情境中，不直接选择最终行动，而是通过有限专注点处理浮现候选卡，使若干行动倾向发生变化；当压力达到临界点后，系统选择最高行动倾向并输出可解释结果。
```

更短定义：

```text
把普通事件选择，从“直接点选结果”，改造成“处理压力下浮现的信息 → 自动生成结果”的最小事件原型。
```
...
| 浮现区 | `emergence_pool` | 当前可处理的候选卡列表 |
| 工作记忆 | `working_memory` | 玩家暂时抓住的候选卡，容量有限 |
| 专注点 | `focus_points` | 每个节点可用的操作预算 |
...
↓
候选卡效果写入 working_memory / action_tendency_tracks / situation_tracks / resolution_log
↓
节点结束，压力上升
↓
进入压力节点 2 / 3
...
```gdscript
{
  "target": "action",       # action / situation / focus / memory / log
...
| 保留 | `keep` | 工作记忆候选卡 | 1 | 下个节点继续保留 |
| 放弃 | `discard` | 工作记忆或浮现区候选卡 | 0 | 移除该候选 |
| 重新浮现 | `refresh` | 当前浮现区 | 1 + 压力上升 | 重新生成候选卡 |
...
```text
observed_instability      对方快失控了
ally_waiting              同伴在等你的判断
hands_shaking             手在发抖
basic_procedure           按流程来
```

典型效果：

```text
对方快失控了：forceful_response +1, pressure +1
同伴在等你的判断：steady_response +1, ally_trust +1
手在发抖：freeze_response +2, forceful_response -1
按流程来：steady_response +2
```

### 8.2 Node 2：说出关键信息

目标：让前期 flag 解锁的证据类候选进入事件。

候选卡建议：

```text
evidence_not_simple       事情不是表面那样
risk_bystander_exposed    旁观者会被卷进来
self_doubt                我可能又搞砸了
keep_talking              继续拖住局面
```
...
```text
事情不是表面那样：steady_response +1, pressure -1, bystander_exposure +1
旁观者会被卷进来：forceful_response +1, bystander_exposure +1
我可能又搞砸了：freeze_response +1
继续拖住局面：steady_response +1, pressure +1
```
...
```text
act_now                   现在必须行动
withdraw_possible         现在后退还来得及
ally_can_hear_you         同伴听得见你
body_locks_up             身体僵住了
```
...
```text
现在必须行动：forceful_response +2, pressure +1
现在后退还来得及：withdraw_response +2
同伴听得见你：steady_response +1, ally_trust +1
身体僵住了：freeze_response +2
```
...
| `result_withdraw` | `withdraw_response` | 角色后退自保，自己风险下降，但局势代价上升 |
...
```text
标题：压力遭遇：节点 1 / 看清局面
选项：抓住【对方快失控了】（专注 -1，强硬干预 +1）
选项：表达【同伴在等你的判断】（专注 -1，同伴信任 +1）
选项：重新浮现（专注 -1，压力 +1）
```
...
5. refresh 必须有压力代价。
6. 最终结果由最高 action_tendency_track 自动生成，而不是玩家直接点击最终结果。
7. 结果日志能解释为什么生成该结果。
8. EventRoom 完成后 GameFlow 可继续推进。
9. BattleDebugScene 不直接维护压力事件规则。
10. 不引入 AGENTS.md 禁止的旧原型体系或平行系统。
```
...
4. BattleDebugScene 直接修改 pressure state。
5. 规则绕过 GameState.submit_choice()。
6. 新增第二套 GameFlow / ChoiceRequest / DebugScene。
7. 使用旧原型禁区命名或恢复旧目录。
8. 自动结果没有可解释日志。
```
...
```text
1. 玩家是否能看懂自己处理的是“当前浮现的信息”，而不是普通手牌？
2. 玩家是否能接受最终行动不是直接选择，而是由行动倾向自动生成？
3. 可解释日志应该显示多少数值，多少自然语言？
4. refresh 的代价应该是压力上升，还是工作记忆污染，还是两者都有？
5. 情绪 / 干扰类候选卡是负担，还是也应该提供某种真实信息价值？
```
