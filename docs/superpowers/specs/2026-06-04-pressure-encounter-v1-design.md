...
```text
v1：通过 EventRoom 启动独立 PressureEncounterState，验证最小压力遭遇循环；不接入默认地图。
v1.1 / v2：在核心循环跑通后，再评估是否把 debug_pressure_encounter 接入默认地图或专用调试地图。
...
15. v1 不接入默认地图；只通过 GUT 与手动构造 EventRoom(debug_pressure_encounter) 验证。
```
...
```gdscript
# Given 一个手动构造的 room_payload.event_id 为 debug_pressure_encounter 的 EventRoom
...
```text
测试或手动 debug 构造 debug_pressure_encounter EventRoom
↓
...
长期目标中，上述循环应逐步成为所有主要房间内容的统一原型；v1 只验证这一循环是否成立，不通过默认地图验证。
...
v1 不接入默认地图，不修改当前默认 7 层地图。

验证方式固定为：

```text
1. GUT 测试中手动构造 EventRoom(debug_pressure_encounter)。
2. 必要时通过手动 debug 入口直接创建 debug_pressure_encounter。
3. 不要求玩家在默认地图路径中走到该节点。
```

理由：

```text
1. v1 已经新增 PressureEncounterState、current_pressure_encounter、pressure_encounter_choice、核心触发、固定自动结算管线等结构。
2. 若同时修改默认地图，会把玩法验证和地图流程接入混在一起，提高排错难度。
3. 当前目标是验证核心循环，不是验证地图内容分布。
```
...
```gdscript
func test_pressure_v1_does_not_require_default_map_node() -> void:
    # Given 默认地图保持当前结构
    # When 测试 debug_pressure_encounter
    # Then 测试应通过手动构造 EventRoom 完成
    # And 不要求默认地图中存在 debug_pressure_encounter 节点
```
...
```text
1. 测试内通过 EventRoom 直接构造 debug_pressure_encounter。
2. 必要时通过手动 debug 入口直接构造 debug_pressure_encounter。
3. 不要求默认地图存在该节点。
```

原因：避免和当前已经验证的默认 7 层地图产生不必要耦合。

默认地图接入留到 v1.1 或 v2，等 Pressure Encounter 核心循环通过测试后再评估。
...
20. v1 测试通过手动构造 EventRoom(debug_pressure_encounter) 完成，不要求默认地图接入。
21. 不引入 AGENTS.md 禁止的旧原型体系或平行系统。
22. 规格表达必须承认：Pressure Encounter 是未来统一框架种子，而不是普通事件旁支。
```
...
23. v1 修改默认地图或强制默认地图接入，导致核心玩法验证和地图流程验证耦合。
```
...
14. 新增可解释日志。
15. 新增 GUT 测试。
16. 通过测试或手动 debug 构造 EventRoom(debug_pressure_encounter) 验证，不接入默认地图。
```
