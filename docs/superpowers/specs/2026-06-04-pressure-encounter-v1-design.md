...
```text
玩家在一个压力情境中，不直接选择最终行动，而是通过有限专注点处理浮现候选卡，使 3 条行动倾向发生变化；候选卡在工作记忆中形成占格、保留、放弃、连锁与污染压力；当压力达到临界点后，系统选择最高行动倾向并输出可解释结果。
```
...
5. 判断是否进入临界结算。
6. 生成 final_result 与 resolution_log。
```
...
| 安抚干扰 | `quiet` | 情绪 / 干扰类候选卡 | 1 | 阻止或移除 emotion_unquieted 累计 |
...
手在发抖：freeze_response +1, emotion_unquieted +1，若 quiet 则移除或阻止该累计
...
旁观者会被卷进来：forceful_response +1, pressure +1, observation +1
我可能又搞砸了：freeze_response +1, emotion_unquieted +1，若 quiet 则移除或阻止该累计
...
身体僵住了：freeze_response +1, emotion_unquieted +1，若 quiet 则移除或阻止该累计
```
...
进入结算前，应先检查 `observation_window` 与 `panic_spiral` 是否触发。
...
```text
1. 列出最终工作记忆与已处理候选。
2. 列出 observation_window 与 panic_spiral 的触发状态。
3. 列出最终行动倾向数值。
4. 选择最高倾向。
5. 套用 pressure / ally_trust 修正。
6. 输出最终结果。
```
...
#### 测试 9：房间完成后回到 GameFlow 并清理当前压力遭遇
...
14. 最终结果由最高 action_tendency_track 自动生成，而不是玩家直接点击最终结果。
15. 结果日志能解释为什么生成该结果，并解释候选池、工作记忆、核心触发和局势的作用。
16. EventRoom 完成后 GameFlow 可继续推进，并清理 current_pressure_encounter。
17. BattleDebugScene 不直接维护压力遭遇规则。
18. 不引入 AGENTS.md 禁止的旧原型体系或平行系统。
19. 规格表达必须承认：Pressure Encounter 是未来统一框架种子，而不是普通事件旁支。
```
...
7. BattleDebugScene 直接修改 pressure state。
8. 规则绕过 GameState.submit_choice()。
9. 新增第二套 GameFlow / ChoiceRequest / DebugScene。
10. 使用旧原型禁区命名或恢复旧目录。
11. 自动结果没有可解释日志。
12. 实现把 Pressure Encounter 写死为普通事件小玩法，阻碍后续统一框架迁移。
13. 试图在 v1 中一次性替换现有 Combat / Rest / Event / Boss，导致范围失控。
14. PressureEncounterState 只作为 EventRoom 字段存在，导致 Combat / Boss / Rest 未来无法复用。
15. GameState 摊平保存 focus_points / working_memory / tracks 等内部字段，变成玩法规则大杂烩。
16. ChoiceResolver 直接维护工作记忆、连锁或最终结算，导致规则分散。
17. v1 重新引入 withdraw_response，导致行动倾向范围失控。
18. v1 重新引入 bystander_exposure 或多 NPC 局势轨，导致局势层过早复杂化。
19. v1 引入第三个及以上 core_trigger，导致 MVP 范围膨胀。
```
...
我处理过的候选卡、刷新代价、工作记忆占格、核心触发和局势变量，都能在结果日志中复盘；
