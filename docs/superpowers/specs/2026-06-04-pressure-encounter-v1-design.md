...
也就是说，未来普通事件、关键剧情、战斗、社交冲突、探索、决战，都应逐步被抽象到同一个核心框架内：
...
注意：下一步审查需要收敛 v1 到底做 1 个 core_trigger，还是做 1 个正向核心 + 1 个污染核心。
...
#### 测试 7：房间完成后回到 GameFlow 并清理当前压力遭遇
...
11. 至少 1 种 chain_tag 或 core_trigger 能影响结算或日志。
12. 最终结果由最高 action_tendency_track 自动生成，而不是玩家直接点击最终结果。
13. 结果日志能解释为什么生成该结果，并解释候选池、工作记忆、连锁和局势的作用。
14. EventRoom 完成后 GameFlow 可继续推进，并清理 current_pressure_encounter。
15. BattleDebugScene 不直接维护压力遭遇规则。
16. 不引入 AGENTS.md 禁止的旧原型体系或平行系统。
17. 规格表达必须承认：Pressure Encounter 是未来统一框架种子，而不是普通事件旁支。
```
...
9. 使用旧原型禁区命名或恢复旧目录。
10. 自动结果没有可解释日志。
11. 实现把 Pressure Encounter 写死为普通事件小玩法，阻碍后续统一框架迁移。
12. 试图在 v1 中一次性替换现有 Combat / Rest / Event / Boss，导致范围失控。
13. PressureEncounterState 只作为 EventRoom 字段存在，导致 Combat / Boss / Rest 未来无法复用。
14. GameState 摊平保存 focus_points / working_memory / tracks 等内部字段，变成玩法规则大杂烩。
15. ChoiceResolver 直接维护工作记忆、连锁或最终结算，导致规则分散。
16. v1 重新引入 withdraw_response，导致行动倾向范围失控。
17. v1 重新引入 bystander_exposure 或多 NPC 局势轨，导致局势层过早复杂化。
```
...
4. v1 是否只需要 1 个 core_trigger，还是应至少包含一个正向核心和一个污染核心？
```
...
9. 新增 refresh 压力代价。
10. 新增 1 个最小 chain_tag / core_trigger。
11. 新增可解释日志。
12. 新增 GUT 测试。
```
