...
本阶段使用 EventRoom，但规格上不得把 Pressure Encounter 写死为事件房专用。

推荐架构：

```text
PressureEncounterState
= 独立玩法运行状态，路径优先 scripts/stm/encounters/pressure/。
...
```text
scripts/stm/encounters/pressure_encounter_state.gd
```

但不建议放在 `scripts/stm/events/`，避免把未来统一遭遇框架写死为事件系统。
...
7. refresh 必须有压力代价。
8. working_memory 必须形成占格压力，discard 必须能释放格子。
9. 至少 1 种 chain_tag 或 core_trigger 能影响结算或日志。
...
7. 新增 refresh 压力代价。
8. 新增 1 个最小 chain_tag / core_trigger。
9. 新增可解释日志。
10. 新增 GUT 测试。
```
