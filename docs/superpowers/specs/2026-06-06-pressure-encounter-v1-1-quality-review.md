...
自动执行环节 = 玩家不再逐项操作，而是系统根据已积攒结构进行集中释放和结算的阶段。
```
...
7. v1 的自动结算更像结尾结果；v1.1 需要把它理解为“压力释放 / 自动执行环节”。
...
6. 增强 resolution_log 的自然语言解释，使其更像自动执行回放。
7. 增加针对机制质量的 GUT 测试。
8. 更新 status 文档。
```
...
```text
从玩家体验看，v1.1 是否需要“同一候选多份 copy”的感觉？
...
- 买走 / 抓住会减少库存；
- discard 会回到库存；
- express 后只在后续压力积攒环节才可能重新进入库存。
```

建议答案：v1.1 先支持数据结构上的 copy_count，但默认每张 1 份。
