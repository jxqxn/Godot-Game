...
7. 增强 resolution_log 的自然语言解释，使其更像自动执行回放。
8. 增加针对机制质量的 GUT 测试。
9. 更新 status 文档。
```
...
```text
未来可以通过 copy_count 或 candidate_family，
让同类恐惧、冲动、执念、怀疑反复浮现，
塑造反刍、强迫性回想或被情绪占据的体验。
```
...
```text
从玩家体验看，自动执行环节应该像“战斗回放”一样展示详细过程，
还是像“红色检定结果”一样突出不可逆的关键后果？
```

建议倾向：两者结合。

```text
先给玩家一个不可逆的关键后果，制造红色检定式释放感；
再按固定管线回放，解释这个后果如何由前面的压力积攒造成。
```
