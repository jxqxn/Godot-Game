...
```text
你结束酒馆操作后，已经不能再干预。
如果你看到插件显示 73% 胜率，你会带着这个预期看自动战斗展开。
快感来自：
- 我知道大概会怎样；
- 但过程仍然会一步步发生；
...
```text
玩家看到：稳住局面 65%，强硬失控 25%，僵住 10%。
然后系统展示角色如何开口、迟疑、被情绪打断、同伴是否跟上、局势是否压住。
玩家不能再改操作，但会带着“这局大概能成”的预期观看自动执行。
```
...
```text
1. 进入不可干预状态。
2. 展示 outcome_rate / 成果率预期。
3. 展示自动执行过程。
4. 展示最终后果。
5. 提供简短原因摘要，解释哪些积攒结构影响了过程。
```
...
7. v1 的自动结算更像结尾结果；v1.1 需要把它理解为“带成果率的自动执行过程展示”。
...
```text
1. LOCK_IN：进入不可干预状态。
2. SHOW_OUTCOME_RATE：展示成果率 / 趋势预期。
3. EXECUTE_SEQUENCE：展示自动执行过程。
4. FINAL_CONSEQUENCE：展示最终后果。
5. CAUSE_SUMMARY：简短解释关键影响来源。
```
...
系统先展示成果率 / 趋势预期。
然后玩家带着这个预期观看事情一步步发生。
快感来自“我知道大概能不能成，但过程仍然会展开并验证或打破我的预期”。
```
...
A. 单一成功率：例如“稳住局面 68%”。
B. 多结果分布：例如“稳住 55% / 强硬 30% / 僵住 15%”。
C. 只显示倾向，不显示百分比：例如“局势大概率会被你稳住，但仍有失控风险”。
```
...
```text
Pressure Encounter 本来就有多条行动倾向轨。
多结果分布比单一成功率更贴近“角色可能以不同方式行动”的心理模拟，
也更像玩家带着胜率插件看自动战斗：不是只看赢不赢，而是看不同结果如何展开。
```
