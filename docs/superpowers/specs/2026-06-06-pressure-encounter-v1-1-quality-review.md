...
```text
文档类型：机制质量审查规格 / v1.1 收敛草案
创建日期：2026-06-06
修订日期：2026-06-06
目标阶段：问答式规格迭代 → 可转实施计划前审查
...
2. v1 已通过完整 GUT。
3. v1.1 继续优先验证机制质量，不急着接默认地图。
4. v1.1 以炉石传说酒馆战棋 10 费回合基础体验为第一参照。
5. 压力积攒环节对应酒馆操作环节。
6. 自动执行环节对应自动团战环节。
7. 大多数普通候选卡 grasp = 暂存 + 占格，不是买入即触发。
8. 少数特殊候选卡可以有即时正负影响，例如 emotion 污染。
9. keep = 冻结关键件，不消耗 focus；成本是继续占用 working_memory。
10. refresh 使用受控随机与有限候选库存池，而不是简单重建列表。
11. express 后候选进入 used，不在当前压力积攒环节立刻回 stock。
12. v1.1 支持数据结构上的 copy_count，但默认每张具体候选 1 份。
13. 重复类似候选主要作为后续塑造反刍、执念和情绪占据的扩展口。
14. 自动执行环节不是事后回放，而是带成果率预期的过程展示。
15. 操作结束时，行动倾向竞争已经决出 dominant_action。
16. outcome_rate 显示的是 dominant_action 的主目标成功率，不是多行动结果分布。
17. 不把结果僵硬画成 Hope/Fear 四象限；Daggerheart 只提供“成功率之外还有过程收益/损失”的启发。
18. 自动执行的重点是：胜率与实际结果之间，还有许多潜在收益和损失需要通过实际过程展示。
19. v1.1 暂时只做 4 类基础自动执行事件：主目标推进、关系协同、情绪干扰、代价/下一轮铺垫。
20. 架构上必须为后续更多事件类型留足空间，不得把事件系统写死成这 4 类。
```
...
v1.1 只实现 4 类基础事件，但架构不能把事件写死成这 4 类。

事件应该被设计成可扩展数据，而不是写死流程分支。
...
```text
1. event_type 可扩展。
2. event_tags 可多标签，方便未来做复仇、亡语、成长、伤害、保护、反刍等事件。
3. source_ids 记录事件来自哪些候选卡、核心、情绪或关系积累。
4. value_delta 描述本次执行赚了什么、亏了什么。
5. next_round_delta 描述下一轮压力积攒环节会受到什么影响。
6. display_text 只负责表现，不承载规则。
7. 事件生成逻辑可以先简单，但事件数据结构不能阻碍后续扩展。
```
...
v1.1 不实现这些复杂类型，但必须避免将来需要重构才能加入。

---

## 12. final_consequence 与 value_summary
...
8. v1 没有 auto_execution_events 数据结构；v1.1 应增加可扩展事件数据。
```
...
9. v1.1 auto_execution_events 只实现 4 类基础事件，但数据结构必须可扩展。
10. 增加 final_consequence / value_summary。
11. 增加针对机制质量的 GUT 测试。
12. 更新 status 文档。
```
...
### 16.6 已确认：v1.1 自动执行事件范围

```text
Q：v1.1 是否先把自动执行事件限制在 4 类？
```
...
```text
Q：虽然 v1.1 只做 4 类事件，架构是否要为后续留足空间？
```

结论：是。

```text
auto_execution_events 必须是可扩展事件数据结构。
不能写死成 4 个 if 分支和 4 种固定文本。
后续应能增加复仇、亡语、成长、保护、站位、资源损失、对方成长、反刍等事件类型。
```
...
```text
从玩家体验看，v1.1 的 4 类自动执行事件中，哪些应该直接影响 value_summary，哪些只负责过程表现？
```

建议倾向：

```text
主目标推进事件：影响 final_consequence。
关系协同事件：可以影响 value_summary，也可以影响下一轮 ally_trust。
情绪干扰事件：可以影响 value_summary，也可以提高下一轮 pressure / freeze 风险。
代价 / 下一轮铺垫事件：必须影响 value_summary 或 next_round_delta。
```
